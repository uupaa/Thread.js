...
Thread["prototype"] = Object.create(Thread, {
    ...
    "post":         { "value": Thread_post  }, // Thread#post(args:AnyArray = null, transfer:TransferableObjectArray = null, postbackHandler:Function = null):void
...
function Thread_post(args,              // @arg AnyArray = null - arguments
                     ...
                     postbackHandler) { // @arg Function = null - postback message handler. postbackHandler(args:AnyArray, event:Event):void
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(args,            "Array|omit"),                   Thread_post, "args");
        $valid($type(transfer,        "TransferableObjectArray|omit"), Thread_post, "transfer");
        $valid($type(postbackHandler, "Function|omit"),                Thread_post, "postbackHandler");
    }
//}@dev

    var pbid = !postbackHandler ? 0 : ++this._postbackCounter;

    if (pbid) {
        this._postbackHandlers[pbid] = postbackHandler; // register postback message handler.
    }
    if (transfer) {
