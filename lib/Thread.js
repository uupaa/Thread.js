...
// --- class / interfaces ----------------------------------
function Thread(script,        // @arg URLString = "" - new Workers(script)
                handlePost,    // @arg Function - handlePost(key:UINT32, value:Any, postback:UINT32, event:Event):void
...
//{@dev
    $valid($type(script, "URLString|omit"), Thread, "script");
    if (_runOnBrowser) {
        ...
        $valid($type(handlePost,  "Function"), Thread, "handlePost");
...
//}@dev
    ...
    if (_runOnBrowser) {
        ...
        this._messageChannel["port1"]["onmessage"] = function(event) {
            if (event.data === "THREAD_CLOSE_READY" || // [3] Thread#close() in MainThread
                event.data === "CLOSED") {             // [6] Thread#close() in WorkerThread
                _close(that, EXIT_OK);
            } else if (event.data === "THREAD_CLOSE_CANCEL") { // [4]
                ...
                if (id) {
                    ...
                    if (handlePost) {
                        handlePost(event.data["key"], event.data["value"], 0, event);
                    }
                }
            }
        };
            ...
            if (event.data === "INIT") {
                ...
                if (handleClose) {
                    handleClose(function() { // ready
                        global["MESSAGE_PORT2"]["postMessage"]("THREAD_CLOSE_READY");  // [3]
                    }, function() { // cancel
                        global["MESSAGE_PORT2"]["postMessage"]("THREAD_CLOSE_CANCEL"); // [4]
                    });
                }
            } else { // MainThread.postMessage({ key, value, postback })
                if (handlePost) {
                    handlePost(event.data["key"],
                               event.data["value"],
...
// utility
Thread["toArrayBuffer"] = Thread_toArrayBuffer; // Thread.toArrayBuffer(source:BlobURLString|URLString|Blob|File|TypedArray|ArrayBuffer, callback:Function):void

// --- implements ------------------------------------------
function Thread_post(key,        // @arg UINT32
                     value,      // @arg Any
                     postback,   // @arg Function|UINT32 = 0 - postback function (in MainThread) function callback is postback(key:UINT32, value:Any, postback:UINT32, event:Event):void
                                 //                         or postback id (in WorkerThread).
...
function Thread_close(timeout) { // @arg Integer = 10000 - watch dog timer, -1 is force close.
    ...
    if (_runOnBrowser) {
        ...
        //      - [3] response a "THREAD_CLOSE_READY" message from WorkerThread
        //          - stop watch dog timer
        //          - clear MessageChannel
        //          - close worker
        //      - [4] response a "THREAD_CLOSE_CANCEL" message from WorkerThread
        //          - stop watch dog timer
...
function Thread_toArrayBuffer(source,     // @arg BlobURLString|URLString|Blob|File|TypedArray|ArrayBuffer
