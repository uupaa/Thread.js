...
// --- dependency modules ----------------------------------
var PORT = "$ThreadProxyMessagingPort";
...
//}@dev
    ...
    function _yes() { global[PORT]["postMessage"]("YES"); } // [3]
    function _no()  { global[PORT]["postMessage"]("NO");  } // [4]
    ...
    global["onmessage"] = function(event) { // @arg Event - event.data
        ...
        switch (data) {
        case "INIT":            global[PORT] = event["ports"][0]; ThreadProxy["TRANSFERBLE"] = true; break;
        case "INIT_X":          global[PORT] = event["ports"][0]; break; // export messageChannel.port2
...
//}@dev
    ...
    if (transfer && ThreadProxy["TRANSFERBLE"]) {
        global[PORT]["postMessage"]({ "pbid": pbid, "args": args }, transfer);
    } else {
        global[PORT]["postMessage"]({ "pbid": pbid, "args": args });
    }
}
...
//}@dev
    ...
    if (transfer && ThreadProxy["TRANSFERBLE"]) {
        global[PORT]["postMessage"]({ "args": args }, transfer);
    } else {
        global[PORT]["postMessage"]({ "args": args });
    }
}

function ThreadProxy_close() {
    global[PORT]["postMessage"]("SELF_CLOSE"); // [6] send a "SELF_CLOSE" message to Thread
